...
class BarConfig {
    ...
    BarConfig(Activity activity) {
        ...
        mStatusBarHeight = getInternalDimensionSize(activity, INSTANCE.getIMMERSION_STATUS_BAR_HEIGHT());
    ...
    private int getNavigationBarHeight(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            if (hasNavBar((Activity) context)) {
                String key;
                if (mInPortrait) {
                    key = INSTANCE.getIMMERSION_NAVIGATION_BAR_HEIGHT();
                } else {
                    key = INSTANCE.getIMMERSION_NAVIGATION_BAR_HEIGHT_LANDSCAPE();
                }
    ...
    private int getNavigationBarWidth(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            if (hasNavBar((Activity) context)) {
                return getInternalDimensionSize(context, INSTANCE.getIMMERSION_NAVIGATION_BAR_WIDTH());
            }
        }
        return result;
    }
    ...
    private boolean hasNavBar(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            if (Settings.Global.getInt(activity.getContentResolver(), INSTANCE.getIMMERSION_MIUI_NAVIGATION_BAR_HIDE_SHOW(), 0) != 0) {
                return false;
            }
            //判断华为手机是否隐藏了导航栏，隐藏了，直接返回false
            if (OSUtils.isEMUI()) {
                if (OSUtils.isEMUI3_x() || Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                    if (Settings.System.getInt(activity.getContentResolver(), INSTANCE.getIMMERSION_EMUI_NAVIGATION_BAR_HIDE_SHOW(), 0) != 0) {
                        return false;
                    }
                } else {
                    if (Settings.Global.getInt(activity.getContentResolver(), INSTANCE.getIMMERSION_EMUI_NAVIGATION_BAR_HIDE_SHOW(), 0) != 0) {
                        return false;
                    }
                }
            }
