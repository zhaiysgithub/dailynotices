...
class MineFansActivity : BaseActivity() {
    ...
    override fun initData() {
        ...
        focusAdapter = RecentVisitAdapter(this@MineFansActivity)
        recyclerView?.setHasFixedSize(true)
        recyclerView?.layoutManager = LinearLayoutManager(this@MineFansActivity)
    ...
    override fun initListener() {
        ...
        smartRefreshLayout?.setOnRefreshListener {
            queryRecentVisitVister(it)
        }

        focusAdapter?.setOnItemClickListener(mFocusItemClick)
    }

    private fun queryRecentVisitVister(it: RefreshLayout) {

        val objectId = PreferenceStorage.userObjectId
        Query.findRecentVisitVisiterList(objectId){ modelList, avException ->
            it.finishRefresh()
            if (avException == null && (modelList?.size ?: 0) > 0) {
                ...
                if(avException != null){
                    ToastUtils.showSafeToast(this@MineFansActivity, errorServer)
                }
            }
