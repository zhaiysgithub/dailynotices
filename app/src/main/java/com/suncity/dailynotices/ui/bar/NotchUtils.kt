...
import com.suncity.dailynotices.ui.bar.BarConstants.Companion.NOTCH_HUA_WEI
import com.suncity.dailynotices.ui.bar.BarConstants.Companion.NOTCH_OPPO
import com.suncity.dailynotices.ui.bar.BarConstants.Companion.NOTCH_VIVO
import com.suncity.dailynotices.ui.bar.BarConstants.Companion.NOTCH_XIAO_MI
import com.suncity.dailynotices.ui.bar.BarConstants.Companion.SYSTEM_PROPERTIES
...
object NotchUtils {
    ...
    fun getNotchHeight(activity: Activity): Int {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && displayCutout != null) {
            if (activity.resources.configuration.orientation == Configuration.ORIENTATION_PORTRAIT) {
                notchHeight = displayCutout.safeInsetTop
            } else {
                if (displayCutout.safeInsetLeft == 0) {
                    notchHeight = displayCutout.safeInsetRight
                } else {
                    notchHeight = displayCutout.safeInsetLeft
                }
            }
        } else {
    ...
    private fun getHuaWeiNotchSize(context: Context): IntArray {
        ...
        try {
            ...
            return get.invoke(aClass) as IntArray
        } catch (ignored: ClassNotFoundException) {
            return ret
        } catch (ignored: NoSuchMethodException) {
            return ret
