 ...
 * @Description:    动态详情页点赞的头像内容区域
 * @UpdateDate:     15/7/2019
 */
open class PileLayout : ViewGroup{

    /**
     * 两个子控件之间的垂直间隙
     */
    private var vertivalSpace: Float = 0.toFloat()

    /**
     * 重叠宽度
     */
    private var pileWidth: Float = 0.toFloat()
    ...
    constructor(context: Context,attrs:AttributeSet?,defStyleAttr:Int):super(context, attrs, defStyleAttr){
        ...
        vertivalSpace = ta.getDimension(R.styleable.PileLayout_PileLayout_vertivalSpace, DisplayUtils.dp2px(4f))
        pileWidth = ta.getDimension(R.styleable.PileLayout_PileLayout_pileWidth, DisplayUtils.dp2px(10f))
        ta.recycle()
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for (i in 0 until count) {
            ...
            val childRowIndex = (if (rowIndex > 0) pileWidth else 0f)
            val childRawWidth = rawWidth + childWidth - childRowIndex
            val childWidthSpaceSize = widthSpecSize - paddingLeft - paddingRight
            if (childRawWidth > childWidthSpaceSize) {
                //换行
                width = Math.max(width, rawWidth)
                rawWidth = childWidth
