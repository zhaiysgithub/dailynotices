 ...
 * @Description: 没有继承HorizontalScrollView不能滑动,对于ViewPager无依赖
 */
open class CommonTabLayout @JvmOverloads constructor(
    ...
    private var mTabEntitys = ArrayList<CustomTabEntity>()
    ...
    var indicatorMarginTop: Float = 0.toFloat()
        private set
    private var indicatorMarginRight: Float = 0.toFloat()
    private var indicatorMarginBottom: Float = 0.toFloat()
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        ...
        indicatorMarginRight =
            ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_right, dp2px(0f).toFloat())
        ...
        mTextSelectsize = ta.getDimension(R.styleable.CommonTabLayout_tl_textSelectsize, sp2px(13f).toFloat())
        mTextUnselectsize = ta.getDimension(R.styleable.CommonTabLayout_tl_textUnselectSize, sp2px(13f).toFloat())
        mTextSelectColor = ta.getColor(R.styleable.CommonTabLayout_tl_textSelectColor, Color.parseColor("#ffffff"))
        mTextUnselectColor =
            ta.getColor(R.styleable.CommonTabLayout_tl_textUnselectColor, Color.parseColor("#AAffffff"))
    ...
    fun setTabData(tabEntitys: ArrayList<CustomTabEntity>) {
        if (tabEntitys.size == 0) {
            LogUtils.e("TabEntitys can not be NULL or EMPTY !")
            return
        }
    ...
    //默认没有需要忽略的位置
    private var ignorePosition: Int = -1

    /** 关联数据支持同时切换fragments 传入需要忽略的位置 */
    fun setTabData(
        tabEntitys: ArrayList<CustomTabEntity>,
        fa: FragmentActivity,
        containerViewId: Int,
        fragments: ArrayList<Fragment?>, ignorePos: Int
    ) {
        mFragmentChangeManager = FragmentChangeManager(fa.supportFragmentManager, containerViewId, fragments)
        ignorePosition = ignorePos
        setTabData(tabEntitys)
    }
    ...
    fun notifyDataSetChanged() {
        ...
        for (i in 0 until tabCount) {
            tabView = when (mIconGravity) {
                Gravity.LEFT -> View.inflate(mContext, R.layout.tab_layout_left, null)
                Gravity.RIGHT -> View.inflate(mContext, R.layout.tab_layout_right, null)
                Gravity.BOTTOM -> View.inflate(mContext, R.layout.tab_layout_bottom, null)
                else -> View.inflate(mContext, R.layout.tab_layout_top, null)
            }
    ...
    /** 创建并添加tab  */
    private fun addTab(pos: Int, tabView: View) {
        val customTabEntity = mTabEntitys[pos]
        val tvTabTitle = tabView.findViewById<View>(R.id.tv_tab_title) as TextView
        tvTabTitle.text = customTabEntity.getTabTitle()
        val ivTabIcon = tabView.findViewById<View>(R.id.iv_tab_icon) as ImageView
        ivTabIcon.setImageResource(customTabEntity.getTabUnselectedIcon())
        if (ignorePosition == pos){
        ...
        mTabsContainer.addView(tabView, pos, lpTab)
    }
    ...
    private fun updateTabStyles() {
        for (i in 0 until tabCount) {
            ...
            if (mIconVisible) {
                ...
                ivTabIcon.setImageResource(if (i == mCurrentTab) tabEntity.getTabSelectedIcon() else tabEntity.getTabUnselectedIcon())
                ...
                when (mIconGravity) {
                    Gravity.LEFT -> lp.rightMargin = mIconMargin.toInt()
                    Gravity.RIGHT -> lp.leftMargin = mIconMargin.toInt()
                    Gravity.BOTTOM -> lp.topMargin = mIconMargin.toInt()
                    else -> lp.bottomMargin = mIconMargin.toInt()
                }
    ...
    private fun updateTabSelection(position: Int) {
        for (i in 0 until tabCount) {
            ...
            val ivTabIcon = tabView.findViewById<View>(R.id.iv_tab_icon) as ImageView
            val tabEntity = mTabEntitys[i]
            ivTabIcon.setImageResource(if (isSelect) tabEntity.getTabSelectedIcon() else tabEntity.getTabUnselectedIcon())
    ...
    fun setMsgMargin(position: Int, leftPadding: Float, bottomPadding: Float) {
        ...
        if (tipView != null) {
            ...
            if (mIconVisible) {
                if (iconH <= 0) {
                    iconH = mContext.resources.getDrawable(mTabEntitys[position].getTabSelectedIcon())
            ...
            if (mIconGravity == Gravity.TOP || mIconGravity == Gravity.BOTTOM) {
                ...
                lp.topMargin =
                    if (mHeight > 0) (mHeight.toFloat() - textHeight - iconH - margin).toInt() / 2 - dp2px(bottomPadding) else dp2px(
                        bottomPadding
                    )
            } else {
                lp.leftMargin = dp2px(leftPadding)
